...
9. For **Unattended `codex exec`**, tmux is required. Create or reuse one detached activity session per plan, `task-graph-<plan-slug>`; the controller remains in `task-graph-controller-<plan-slug>`. After writing the task brief, use `launch-exec` for each reserved task in its dedicated worktree; it creates a named worker window in the plan session, writes a durable runtime record before execution, captures output in a task-specific log, and preserves the exited pane for diagnosis. Reviews and repairs use their own named windows in the same session. Serialize plan-session window creation and removal. It runs `codex exec` with the normal workspace-write sandbox and the installed CLI's execution policy; never automatically use `--dangerously-bypass-approvals-and-sandbox`. The launcher persists the worker's final response to the controller-owned report path with `--output-last-message`; require that response to include one of the task status values below, a summary, tests, concerns, and a suggested commit message. The worker must edit and test only in its task worktree, and must not write run artifacts or commit. `DONE` means the task is ready for review, not cleanup: retain its worktree and tmux window through diff inspection, verification, task-commit creation, and integration. The runtime record contains the plan tmux session, stable window identifier, pane PID (the process identifier), command, worktree, branch, brief/report/log paths, start/finish timestamps, and exit result. On resume, inspect the runtime record, report, and log before retrying; do not relaunch a completed task. Local `codex exec` still requires an awake machine or remote host.
10. Monitor each unattended local worker with low intrusion:
   ...
   - A quiet checkpoint exits `124` after its `--seconds` bound. It is read-only and must not reserve, relaunch, move, deliver, or clean up work.
    ...
    - `REPAIR_REQUIRED` is not a reminder: durably reserve the repair child run, branch, and worktree, then launch the inherited worker mode with a brief limited to the reviewer findings. Only a valid child runtime record consumes the automatic repair; a restart must reuse the reservation. Preserve incomplete setup artifacts, and surface `INSPECTION_REQUIRED` rather than deleting an artifact that conflicts with its reservation.
...
13. Handle every result from the entire currently unblocked batch before launching its dependents:
   ...
   - For `NEEDS_CONTEXT`, provide the missing context and re-dispatch the same task.
    ...
    - After a successful report, approved review, and test evidence, run `delivery-ready` to select the permitted controller action. `no-mistakes` requires its full pipeline, `direct-pr` opens a PR, and `local-only` fast-forwards only a clean integration branch. With +yolo, the controller may merge a green PR or fast-forward locally; otherwise it asks for approval. After the task commit is integrated and verification passes, record it with `record-delivery --result landed`, then immediately run teardown before marking that task done. Teardown removes both the dedicated worktree and recorded tmux window; it happens per task and does not wait for the plan's final PR. The shared plan session disappears naturally after its final window closes. Retain failed or retrying windows for diagnosis, and use `--discard` only after explicitly abandoning unlanded work.
...
- `launch-exec --plan <plan-slug> --run-id <id> --task <file> --branch <branch> --worktree <path>` requires tmux and starts one reserved in-progress task in its named window of the shared `task-graph-<plan-slug>` session.
...
- `controller.py run --plan <plan-slug>` is the tmux child. It drains claimed wakes before new wakes, starts fresh read-only reviews or the single focused repair, and acknowledges a wake only once the session starts. Human-required and delivery/tooling outcomes become durable `escalated` wakes and pause the controller until the underlying task condition is resolved. `SUPERVISION_STATE_CORRUPTION` means queue, claims, or dedupe state is malformed; repair or replace the named artifact. Use `controller.py repair-wake-queue --repo <repo-root> --plan <plan-slug>` only for a stopped, non-live controller paused on `wake-queue.jsonl` corruption. It snapshots the queue and retains only schema-valid wakes; then explicitly run `controller.py start`. Never auto-repair the artifact or auto-restart the controller.
- A worker launch records durable `intent` before tmux creation and records `launched` only after creation succeeds. A transient named-window conflict gets one locked re-check and one retry. A target that persists, or an ambiguous retry failure, remains a retained conflicted runtime and requires `INSPECTION_REQUIRED`; never kill, reuse, or overwrite the existing window.
